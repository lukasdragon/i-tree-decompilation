...
// Type: i_Tree_Eco_v6.Reports.NetCarbonSequestrationByStrataPerUnitArea
...
{
  internal class NetCarbonSequestrationByStrataPerUnitArea : DatabaseReport
  {
    public NetCarbonSequestrationByStrataPerUnitArea() => this.ReportTitle = i_Tree_Eco_v6.Resources.Strings.ReportTitleAnnualNetCarbonSequestrationOfTreesByStratumperUnitArea;
    ...
    public override void RenderBody(C1PrintDocument C1doc, Graphics g)
    {
      ...
      ChartIn.ChartArea.AxisY.Text = ReportUtil.FormatInlineHeaderUnitsStr(i_Tree_Eco_v6.Resources.Strings.NetCarbonSequestrationDensity, ReportUtil.GetValuePerValueStr(ReportUtil.GetValuePerYrStr(ReportBase.KgUnits()), ReportBase.HaUnits()));
      ...
      foreach (DataRow row in (InternalDataCollectionBase) carbonSequestration.Rows)
      {
        ...
        if (carbonSequestration.Rows.Count > 1 && str != "Study Area" || carbonSequestration.Rows.Count == 1)
        {
          ...
          chartDataSeries1.Y.Add((object) EstimateUtil.ConvertToEnglish(ReportUtil.ConvertFromDBVal<double>(row["EstimateValue"]) * 1000.0, Tuple.Create<Units, Units, Units>(Units.Kilograms, Units.Year, Units.Hectare), ReportBase.EnglishUnits));
      ...
      ChartIn.ChartArea.AxisY2.Text = ReportUtil.FormatInlineHeaderUnitsStr(i_Tree_Eco_v6.Resources.Strings.CO2Equivalent, ReportUtil.GetValuePerValueStr(ReportUtil.GetValuePerYrStr(ReportBase.KgUnits()), ReportBase.HaUnits()));
      ...
      renderTable.Cells[0, 1].Text = i_Tree_Eco_v6.Resources.Strings.NetCarbonSequestrationDensity;
      renderTable.Cells[0, 2].Text = i_Tree_Eco_v6.Resources.Strings.CO2Equivalent;
      renderTable.Cells[1, 1].Text = renderTable.Cells[1, 2].Text = ReportUtil.FormatHeaderUnitsStr(ReportUtil.GetValuePerValueStr(ReportUtil.GetValuePerYrStr(ReportBase.KgUnits()), ReportBase.HaUnits()));
      int num3 = count;
      foreach (DataRow row in (InternalDataCollectionBase) carbonSequestration.Rows)
      {
        string str = this.estUtil.ClassValues[Classifiers.Strata][(short) ReportUtil.ConvertFromDBVal<int>(row[classifierName])].Item1;
        renderTable.Cells[num3, 0].Text = str;
        ...
        double english = EstimateUtil.ConvertToEnglish(ReportUtil.ConvertFromDBVal<double>(row["EstimateValue"]) * 1000.0, Tuple.Create<Units, Units, Units>(Units.Kilograms, Units.Year, Units.Hectare), ReportBase.EnglishUnits);
