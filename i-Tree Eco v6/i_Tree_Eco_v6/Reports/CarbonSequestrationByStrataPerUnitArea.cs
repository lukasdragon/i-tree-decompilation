...
// Type: i_Tree_Eco_v6.Reports.CarbonSequestrationByStrataPerUnitArea
...
{
  internal class CarbonSequestrationByStrataPerUnitArea : DatabaseReport
  {
    public CarbonSequestrationByStrataPerUnitArea() => this.ReportTitle = i_Tree_Eco_v6.Resources.Strings.ReportTitleAnnualCarbonSequestrationOfTreesByStratumPerUnitArea;

    public override void RenderBody(C1PrintDocument C1doc, Graphics g)
    {
      SortedList<int, double> sequestrationSortedList = this.GetCarbonSequestrationSortedList(this.GetCarbonSequestrationWithTotals());
      if (sequestrationSortedList.Count == 2 && this.estUtil.ClassValues[Classifiers.Strata][(short) sequestrationSortedList.Keys[1]].Item1 == "Study Area")
        sequestrationSortedList.RemoveAt(1);
      string valuePerValueStr = ReportUtil.GetValuePerValueStr(ReportUtil.GetValuePerYrStr(ReportBase.KgUnits()), ReportBase.HaUnits());
      RenderTable renderTable = new RenderTable();
      C1doc.Body.Children.Add((RenderObject) renderTable);
      renderTable.Width = (Unit) "75%";
      renderTable.Cols[0].Width = (Unit) "15%";
      renderTable.Cols[1].Width = (Unit) "35%";
      int count = 2;
      ...
      renderTable.Cells[0, 1].Text = i_Tree_Eco_v6.Resources.Strings.GrossCarbonSequestrationDensity;
      renderTable.Cells[1, 1].Text = ReportUtil.FormatHeaderUnitsStr(valuePerValueStr);
      renderTable.Cells[0, 2].Text = i_Tree_Eco_v6.Resources.Strings.CO2Equivalent;
      renderTable.Cells[1, 2].Text = ReportUtil.FormatHeaderUnitsStr(valuePerValueStr);
      int row = count;
      for (int index = 0; index < sequestrationSortedList.Count; ++index)
      {
        ...
        renderTable.Cells[row, 0].Text = str;
        if (str == "Study Area")
        {
          renderTable.Cells[index + count, 0].Text = i_Tree_Eco_v6.Resources.Strings.StudyArea;
          renderTable.Rows[index + count].Style.FontBold = true;
          renderTable.Rows[index + count].Style.Borders.Top = LineDef.Default;
        }
        double english = EstimateUtil.ConvertToEnglish(sequestrationSortedList.Values[index], Tuple.Create<Units, Units, Units>(Units.Kilograms, Units.Year, Units.Hectare), ReportBase.EnglishUnits);
        renderTable.Cells[row, 1].Text = english.ToString("N2");
        renderTable.Cells[row, 2].Text = (english * 3.667).ToString("N2");
        ++row;
      }
      ...
      ChartIn.ChartArea.AxisY.Text = ReportUtil.FormatInlineHeaderUnitsStr(i_Tree_Eco_v6.Resources.Strings.GrossCarbonSequestrationDensity, valuePerValueStr);
      ...
      ChartIn.ChartArea.AxisY.AnnoFormat = FormatEnum.NumericManual;
      ChartIn.ChartArea.AxisY.AnnoFormatString = "N0";
      ChartIn.ChartArea.AxisY2.AnnoFormat = FormatEnum.NumericManual;
      ChartIn.ChartArea.AxisY2.AnnoFormatString = "N0";
      ...
      {
        string str = this.curYear.RecordStrata || sequestrationSortedList.Count != 1 ? this.estUtil.ClassValues[Classifiers.Strata][(short) sequestrationSortedList.Keys[index]].Item1 : "Study Area";
        if (sequestrationSortedList.Keys.Count > 1 && str != "Study Area" || sequestrationSortedList.Count == 1)
        {
          chartDataSeries1.X.Add((object) index);
          chartDataSeries1.Y.Add((object) EstimateUtil.ConvertToEnglish(sequestrationSortedList.Values[index], Tuple.Create<Units, Units, Units>(Units.Kilograms, Units.Year, Units.Hectare), ReportBase.EnglishUnits));
      ...
      ChartIn.ChartArea.AxisY2.Text = ReportUtil.FormatInlineHeaderUnitsStr(i_Tree_Eco_v6.Resources.Strings.CO2Equivalent, valuePerValueStr);
      ...
      RenderObject chartRenderObject = (RenderObject) ReportUtil.CreateChartRenderObject(ChartIn, g, C1doc, 1.0, 0.8);
